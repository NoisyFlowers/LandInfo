...
public class AlarmReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
        if (BOOT_ACTION.equals(intent.getAction())) { 
			Log.d(DEBUG_TAG, "LPKS Boot alarm, scheduling sync alarms");	
			LandPKSApplication.getInstance().setRecurringAlarm(context);
        } else {
			Log.d(DEBUG_TAG, "LPKS Recurring alarm, starting sync service");	
			LandPKSApplication.getInstance().startSyncService(context);
        }
	}
}

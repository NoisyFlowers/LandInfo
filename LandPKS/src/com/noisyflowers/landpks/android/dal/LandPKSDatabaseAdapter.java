...
public class LandPKSDatabaseAdapter {
	...
	***/
	...
	private static final String PLOTS_TABLE_CREATE = "create table " + PLOTS_TABLE + " (" +
		...
		"soil_profile_bedrock_cm integer," +
		"soil_profile_stopped_digging_cm integer," +
	...
	***/
	...
	private Plot loadPlot(Cursor resultSet) {
		...
		if (!resultSet.isNull(resultSet.getColumnIndex("grazing"))) {
			String grazingCSV = resultSet.getString(resultSet.getColumnIndex("grazing"));
			String[] grazingTypes = grazingCSV.split(",");
			plot.grazing = new ArrayList<Plot.Grazing>();
			for (int x = 0; x < grazingTypes.length; x++) {
				plot.grazing.add(Plot.Grazing.valueOf(grazingTypes[x]));
			}
		}
		...
		Integer bedrock = null, stoppedDigging = null;
		if (!resultSet.isNull(resultSet.getColumnIndex("soil_profile_bedrock_cm"))) {
			bedrock = resultSet.getInt(resultSet.getColumnIndex("soil_profile_bedrock_cm"));
		} else if (!resultSet.isNull(resultSet.getColumnIndex("soil_profile_stopped_digging_cm"))) {
			stoppedDigging = resultSet.getInt(resultSet.getColumnIndex("soil_profile_stopped_digging_cm"));
		}
        for(HorizonName h : HorizonName.values()) {
    		String[] bounds = h.name.split("[^\\d]");
			...
			if (bedrock != null && bedrock >= lowVal && bedrock <= highVal) {
				horizon.bedrock = bedrock;
			} else if (stoppedDigging != null && stoppedDigging >= lowVal && stoppedDigging <= highVal) {
				horizon.stoppedDigging = stoppedDigging;
			}
			
			if (horizon.rockFragment != null || horizon.color != null || horizon.texture != null || horizon.bedrock != null || horizon.stoppedDigging != null)
				plot.soilHorizons.put(h.name, horizon); 
			
			if (horizon.bedrock != null || horizon.stoppedDigging != null)
				break;//don't care about values after stopped
        }
	...
	public void addPlot(Plot plot) {
		...
		try {
	    	...
	    	if (plot.grazing != null && plot.grazing.size() > 0) {
	    		StringBuilder grazingCSV = new StringBuilder();
	    		for (Plot.Grazing grazingType : plot.grazing) {
	    			grazingCSV.append(grazingType.name() + ",");
	    		}
		    	values.put("grazing", grazingCSV.toString());
	    	}
	        ...
	        for(HorizonName h : HorizonName.values()) {
				...
				horizon = horizon == null ? new SoilHorizon() : horizon;
				String horizonNumber = "" + (h.ordinal() + 1);
	    		...
	    		if (horizon.bedrock != null) {
	    			values.put("soil_profile_bedrock_cm", horizon.bedrock);
	    			values.putNull("soil_profile_stopped_digging_cm");
	    			break; //not interested in horizon values after stop
	    		} else {
	    			values.putNull("soil_profile_bedrock_cm");	    			
	    		}  
	    		if (horizon.stoppedDigging != null) {
	    			values.put("soil_profile_stopped_digging_cm", horizon.stoppedDigging);
	    			values.putNull("soil_profile_bedrock_cm");
	    			break; //not interested in horizon values after stop
	    		} else {
	    			values.putNull("soil_profile_stopped_digging_cm");
	...
	private class LandPKSDBHelper extends SQLiteOpenHelper {
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion){
			...
			switch (oldVersion) {
				...
				case 20:
					try {
					    db.execSQL("ALTER TABLE " + PLOTS_TABLE + " add column grazing text");
					} catch (Exception eX) {
						Log.e(TAG, "Unable to add column grazing", eX);
						hosed = true;
					}
					try {
					    db.execSQL("ALTER TABLE " + PLOTS_TABLE + " add column soil_profile_bedrock_cm integer");
					} catch (Exception eX) {
						Log.e(TAG, "Unable to add column column soil_profile_stopped_digging_cm", eX);
						hosed = true;
					}
					try {
					    db.execSQL("ALTER TABLE " + PLOTS_TABLE + " add column soil_profile_stopped_digging_cm integer");
					} catch (Exception eX) {
						Log.e(TAG, "Unable to add column column soil_profile_stopped_digging_cm", eX);
						hosed = true;
					}

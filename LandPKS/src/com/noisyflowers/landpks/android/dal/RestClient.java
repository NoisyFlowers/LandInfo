...
public class RestClient {
	...
	public com.noisyflowers.landpks.android.model.Plot getQuickClimate(com.noisyflowers.landpks.android.model.Plot plot) {
		com.noisyflowers.landpks.android.model.Plot result = new Plot();
		result.latitude = plot.latitude;
		result.longitude = plot.longitude;
    	...
    	Plotendpoint endpoint = buildPlotEndpoint();
    	try {
	    	remotePlot = endpoint.quickClimate(remotePlot).execute();
			...
			Log.e(TAG, "Error getting quick climate.", eX);
			result = null;
		}
	...
	public com.noisyflowers.landpks.android.model.Plot putPlot(com.noisyflowers.landpks.android.model.Plot plot) {
    	...
    	if (plot.grazing != null && plot.grazing.size() > 0) {
    		List<String> remoteGrazingList = new ArrayList<String>();
    		for (Plot.Grazing grazingType : plot.grazing) {
    			remoteGrazingList.add(grazingType.name());
    		}
        	remotePlot.setGrazing(remoteGrazingList);    		
    	}
	    	...
	    	if (horizon != null) {
	    		try {
		    		...
		    		if (horizon.bedrock != null) {
		    			remotePlot.setBedrockDepth(horizon.bedrock);
		    			remotePlot.setStoppedDiggingDepth(null);
		    			break; //not interested in horizon values after stop
		    		} else if (horizon.stoppedDigging != null) {
		    			remotePlot.setStoppedDiggingDepth(horizon.stoppedDigging);
		    			remotePlot.setBedrockDepth(null);
		    			break; //not interested in horizon values after stop
		    		}
		    	} catch (Exception eX) {
		...
		if (plots != null) {
			...
			if (plots.getItems() != null) {
				for (com.noisyflowers.landpks.server.gae.model.plotendpoint.model.Plot remotePlot: plots.getItems()) {
			    	...
			    	if (remotePlot.getGrazing() != null && remotePlot.getGrazing().size() > 0) {
			    		List<Plot.Grazing> grazingList = new ArrayList<Plot.Grazing>();
			    		for (String grazingStr : remotePlot.getGrazing()) {
			    			grazingList.add(Plot.Grazing.valueOf(grazingStr));
			    		}
			    	}
					...
					for(HorizonName h : HorizonName.values()) {
			    		...
			    		try {
					    	...
					    	Integer bedrock = remotePlot.getBedrockDepth(), stoppedDigging = remotePlot.getStoppedDiggingDepth();
